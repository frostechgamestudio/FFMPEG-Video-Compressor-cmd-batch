...
for /r "Input" %%F in (*.mp4 *.avi *.mkv *.mov *.wmv *.webm *.flv *.m4v *.ts *.mts *.mpeg *.mpg) do (
    ...
    if /I "!FORMAT!"=="h264" (
        ...
        ffmpeg %BASE_PARAMS% %HWACCEL_PARAMS% -i "%%F" %VIDEO_FILTER% -c:v hevc_amf -preset quality -profile:v main -quality quality -usage high_quality -qp %QUALITY% -r %FPS% %AUDIO_PARAMS% %OUTPUT_PARAMS% -y "!tempHevcFile!"

        if !ERRORLEVEL! equ 0 (
            echo   Process 2/2: H.264 encoding from HEVC...
            ffmpeg %BASE_PARAMS% %HWACCEL_PARAMS% -i "!tempHevcFile!" %VIDEO_FILTER% -c:v h264_amf -preset quality -profile:v main -quality quality -usage high_quality -qp %QUALITY% %AUDIO_PARAMS% %OUTPUT_PARAMS% -y "!outputFile!"
        ...
        set "outputFile=!outputDir!!fileName!.mp4"
        ffmpeg %BASE_PARAMS% %HWACCEL_PARAMS% -i "%%F" %VIDEO_FILTER% -c:v hevc_amf -preset quality -profile:v main -quality quality -usage high_quality -qp %QUALITY% -r %FPS% %AUDIO_PARAMS% %OUTPUT_PARAMS% -y "!outputFile!"
    )
